resource "azurerm_virtual_machine_scale_set_extension" "devops_selfhosted_agent" {
  for_each = {
  ...
  protected_settings = jsonencode(
    {
      "commandToExecute" : format("bash %s '%s' '%s' '%s' '%s' '%s' '%s' '%s'", var.extensions[each.key].agent_init_script, var.settings[each.key].azure_devops.url, var.settings[each.key].agent_pat, try(var.settings[each.key].azure_devops.agent_pool.name, ""), try(var.settings[each.key].azure_devops.agent_pool.agent_name_prefix, ""), try(var.settings[each.key].azure_devops.agent_pool.num_agents, ""), var.settings[each.key].admin_username, var.settings[each.key].azure_devops.rover_version)
    }
  )
}
